    ...
    function renderPanelContent() {
        ...
        FIELD_CONFIG.forEach(item => {
            ...
            if (value && String(value).trim()) {
                ...
                // Click on the row to copy - no separate button needed
                html += `
                    <div class="ffh-field clickable" data-value="${displayValue.replace(/"/g, '&quot;')}" data-label="${item.label}" title="Click to copy ${item.label}">
                        <span class="ffh-field-label">${item.label}</span>
                        <span class="ffh-field-value">${displayValue}</span>
                    </div>
                `;
            }
        });
        ...
        // Add click handlers to copy on click
        body.querySelectorAll('.ffh-field.clickable').forEach(field => {
            field.addEventListener('click', (e) => {
                e.stopPropagation();
                copyToClipboardSimple(field.dataset.value, field.dataset.label, field);
            });
        });
    }

    // Simple copy to clipboard - shows field label in toast
    async function copyToClipboardSimple(text, label, element) {
        try {
            await navigator.clipboard.writeText(text);
            showToast(`✓ ${label} copied! Paste with Ctrl+V`);
